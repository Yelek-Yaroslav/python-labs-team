...
def file_answer4():
    # Шевельов Артем: функція додає відповідь на питання
    try:
        with open("team_file.txt", "a", encoding="utf-8") as file:
            file.write("Студент 4: Шевельов Артем\n")
            file.write("Відповідь на питання 3:\n")
            file.write(
                "Масив (array) у Python використовується для зберігання впорядкованої "
                "послідовності однотипних елементів. На відміну від списків, масиви "
                "з модуля array зберігають дані одного типу, що дозволяє ефективніше "
                "використовувати пам’ять.\n"
            )
            file.write(
                "Масиви підтримують доступ до елементів за індексом, проходження циклом "
                "та базові операції з даними. Найчастіше масиви застосовуються у задачах, "
                "де важлива швидкість обробки числових даних або робота з великими обсягами "
                "однотипної інформації.\n\n"
            )
        print("Студент 4: відповідь додана.")
    except Exception as e:
        file_error_check(e)
